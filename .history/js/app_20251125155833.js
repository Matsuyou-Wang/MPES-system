    ...
    setupEventListeners() {
        ...
        if (replaySegmentBtn) {
            replaySegmentBtn.addEventListener('click', () => {
                const clipPlayer = document.getElementById('clipPlayer');
                if (clipPlayer && clipPlayer.dataset.fStart) {
                    const fStart = parseFloat(clipPlayer.dataset.fStart);
                    const mEnd = parseFloat(clipPlayer.dataset.mEnd);
                    
                    console.log(`重播小窗片段: ${fStart} 到 ${mEnd}`);
                    ...
                    // 设置小窗视频到开始位置并播放
                    clipPlayer.currentTime = fStart;
                    clipPlayer.play();
                    
                    // 监听播放,到达结束时间时暂停
                    const segmentEndHandler = () => {
                        if (clipPlayer.currentTime >= mEnd) {
                            clipPlayer.pause();
                            clipPlayer.currentTime = fStart;
                        }
                    };
