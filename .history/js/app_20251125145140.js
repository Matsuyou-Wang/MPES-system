...
class AnnotationSystem {
    ...
    showAnnotationModal(point) {
        ...
        if (clipPlayer && this.currentVideo) {
            ...
            // 移除旧的事件监听器
            if (clipPlayer._handleClipTimeUpdate) {
                clipPlayer.removeEventListener('timeupdate', clipPlayer._handleClipTimeUpdate);
            }
            if (clipPlayer._handleClipLoadedMetadata) {
                clipPlayer.removeEventListener('loadedmetadata', clipPlayer._handleClipLoadedMetadata);
            }
            
            // 创建新的事件处理函数
            const handleClipTimeUpdate = () => {
                const start = parseFloat(clipPlayer.dataset.startTime);
                const end = parseFloat(clipPlayer.dataset.endTime);
                
                // 如果超出范围,暂停并重置到开始
                if (clipPlayer.currentTime >= end) {
                    clipPlayer.pause();
                    clipPlayer.currentTime = start;
                }
                
                // 如果小于开始时间,跳到开始
                if (clipPlayer.currentTime < start) {
                    clipPlayer.currentTime = start;
                }
            };
            
            const handleClipLoadedMetadata = () => {
                clipPlayer.currentTime = startTime;
            };
            
            // 保存引用并添加监听器
            clipPlayer._handleClipTimeUpdate = handleClipTimeUpdate;
            clipPlayer._handleClipLoadedMetadata = handleClipLoadedMetadata;
            clipPlayer.addEventListener('timeupdate', handleClipTimeUpdate);
            clipPlayer.addEventListener('loadedmetadata', handleClipLoadedMetadata);
        ...
        if (replaySegmentBtn) {
            replaySegmentBtn.addEventListener('click', () => {
                if (this.currentAnnotationPoint) {
                    ...
                    console.log(`Replaying segment from ${startTime}s to ${endTime}s`);
                    
                    // 移除之前的监听器（如果有）
                    if (this.segmentCheckHandler) {
                        this.videoPlayer.removeEventListener('timeupdate', this.segmentCheckHandler);
                    }
                    ...
                    // 监听时间更新，到达结束时间时暂停
                    this.segmentCheckHandler = () => {
                        if (this.videoPlayer.currentTime >= endTime) {
                            this.videoPlayer.pause();
                            this.videoPlayer.currentTime = endTime;
                            this.videoPlayer.removeEventListener('timeupdate', this.segmentCheckHandler);
                            this.segmentCheckHandler = null;
                        }
                    };
                    this.videoPlayer.addEventListener('timeupdate', this.segmentCheckHandler);
                }
            });
        }
