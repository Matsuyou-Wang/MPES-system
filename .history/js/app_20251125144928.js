...
class AnnotationSystem {
    ...
    setupEventListeners() {
        ...
        if (replayClipBtn) {
            replayClipBtn.addEventListener('click', () => {
                if (this.currentAnnotationPoint) {
                    const point = this.currentAnnotationPoint;
                    // 从第一个speaker的start time播放到第二个speaker的end time
                    const startTime = point.f_start || point.m_start || (point.time - 5);
                    const endTime = point.m_end || point.f_end || point.time;
                    ...
                    const pauseAtEnd = () => {
                        if (this.videoPlayer.currentTime >= endTime) {
                            this.videoPlayer.pause();
                            this.videoPlayer.removeEventListener('timeupdate', pauseAtEnd);
                        }
                    };
                    this.videoPlayer.addEventListener('timeupdate', pauseAtEnd);
                }
            });
        }
