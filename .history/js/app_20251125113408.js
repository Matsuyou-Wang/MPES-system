...
class AnnotationSystem {
    ...
    // 显示下一个标注点
    showNextAnnotationPoint() {
        if (!this.annotationPoints || this.annotationPoints.length === 0) return;
        
        // 找到当前标注点的索引
        const currentIndex = this.annotationPoints.findIndex(p => p === this.currentAnnotationPoint);
        const nextIndex = currentIndex + 1;
        
        if (nextIndex < this.annotationPoints.length) {
            const nextPoint = this.annotationPoints[nextIndex];
            this.currentAnnotationPoint = nextPoint;
            setTimeout(() => {
                this.showAnnotationModal(nextPoint);
            }, 500); // 短暂延迟
        } else {
            console.log('所有标注点已完成');
            alert('恭喜！您已完成所有标注点。');
            // 返回到视频选择界面
            this.annotationView.classList.remove('active');
            this.selectionView.classList.add('active');
        }
    }

    // 继续到下一个标注点
    continuePlayback() {
        console.log('继续到下一个标注点');
        this.isModalOpen = false;
        this.modal.style.display = 'none';
        
        // 显示下一个标注点
        this.showNextAnnotationPoint();
    }
