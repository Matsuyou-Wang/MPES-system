...
def process_dialog_excel():
    ...
    if os.path.exists(f_file):
        ...
        for filename, group in df_f.groupby('文件名'):
            ...
            for _, row in group.iterrows():
                # 对于F视频，Female先说，Male回应 → 评估第二个说话者（Male）
                # 只在M_end时创建评估点，评估男性说话者（第二个说话者）
                if pd.notna(row['M_end']):
                    dialog_points.append({
                        'time': float(row['M_end']),
                        'type': 'M_end',  # F视频评估男性说话者（第二个说话者）
                        'f_text': str(row['F_text']) if pd.notna(row['F_text']) else '',
                        'm_text': str(row['M_text']) if pd.notna(row['M_text']) else '',
                        'f_start': float(row['F_start']) if pd.notna(row['F_start']) else None,
                        'm_start': float(row['M_start']) if pd.notna(row['M_start']) else None,
                        'f_end': float(row['F_end']) if pd.notna(row['F_end']) else None,
                        'm_end': float(row['M_end']) if pd.notna(row['M_end']) else None
                    })
    ...
    if os.path.exists(m_file):
        ...
        for filename, group in df_m.groupby('文件名'):
            ...
            for _, row in group.iterrows():
                # 对于M视频，Male先说，Female回应 → 评估第二个说话者（Female）
                # 只在F_end时创建评估点，评估女性说话者（第二个说话者）
                if pd.notna(row['F_end']):
                    dialog_points.append({
                        'time': float(row['F_end']),
                        'type': 'F_end',  # M视频评估女性说话者（第二个说话者）
                        'm_text': str(row['M_text']) if pd.notna(row['M_text']) else '',
                        'f_text': str(row['F_text']) if pd.notna(row['F_text']) else '',
                        'm_start': float(row['M_start']) if pd.notna(row['M_start']) else None,
                        'f_start': float(row['F_start']) if pd.notna(row['F_start']) else None,
