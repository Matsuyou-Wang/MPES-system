...
def process_dialog_excel():
    ...
    for filename, group in df.groupby('文件名'):
        ...
        # 同时为对应的男性视频创建数据
        # Ses01F_script01_1.mp4 -> Ses01M_script01_1.mp4
        if 'F_' in video_filename:
            male_video_filename = video_filename.replace('F_', 'M_')
            # 男性视频使用相同的对话点数据
            dialog_data[male_video_filename] = dialog_points
